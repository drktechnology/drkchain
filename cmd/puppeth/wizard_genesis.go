...
import (
	...
	"github.com/ethereum/go-ethereum/contracts/deployer"
	"github.com/ethereum/go-ethereum/contracts/rchain/contract"
...
func (w *wizard) makeGenesis() {
	...
	fmt.Println()
	fmt.Println("Which consensus engine to use? (default = SPOS)")
	fmt.Println(" 1. Ethash - proof-of-work")
	fmt.Println(" 2. Clique - proof-of-authority")
	fmt.Println(" 3. SPOS   - proof-of-stake")
	...
	case choice == "" || choice == "3":
		// In the case of clique, configure the consensus parameters
		genesis.Difficulty = big.NewInt(1)
		genesis.GasLimit = 80000000
		genesis.Config.Spos = &params.SPOSConfig{
			Period: 2,
			Epoch:  600,
		}
		fmt.Println()
		fmt.Println("How many seconds should blocks take? (default = 2)")
		genesis.Config.Spos.Period = uint64(w.readDefaultInt(2))
		...
		// We also need the initial list of beneficiaries
		var beneficiaries []common.Address
		for i := 0; i < len(signers); i++ {
			fmt.Println()
			fmt.Printf("Which account is beneficiary for signer: %v\n", signers[i])
			for {
				if address := w.readAddress(); address != nil {
					beneficiaries = append(beneficiaries, *address)
					break
				}
			}
		}
		...
		fmt.Println()
		fmt.Println("How many fund will be pre-mined for validators' deposit (default = 50K)")

		prefund := new(big.Int).Mul(w.readDefaultBigInt(big.NewInt(50000)), big.NewInt(1e+18))

		prefunds := make([]*big.Int, len(signers))
		for i := 0; i < len(prefunds); i++ {
			prefunds[i] = prefund
		}

		// Ask for initial funds
		var genesisPrefundAddr []common.Address
		var genesisPrefundBalance []*big.Int
		fmt.Println()
		fmt.Println("Which accounts should be pre-funded? (advisable at least one)")
		for {
			// Read the address of the account to fund
			if address := w.readAddress(); address != nil {
				fmt.Printf("How many fund for %v?\n", address)
				fund := new(big.Int).Mul(w.readDefaultBigInt(big.NewInt(1)), big.NewInt(1e+18))
				genesis.Alloc[*address] = core.GenesisAccount{
					Balance: fund,
				}
				genesisPrefundAddr = append(genesisPrefundAddr, *address)
				genesisPrefundBalance = append(genesisPrefundBalance, fund)
				continue
			}
			break
		}
		genesisPrefundAddr = append(genesisPrefundAddr, params.ValidatorContract)
		genesisPrefundBalance = append(genesisPrefundBalance, new(big.Int).Mul(prefund, big.NewInt(int64(len(signers)))))

		code, storage, err := deployer.DeployContract(func(sim *backends.SimulatedBackend, auth *bind.TransactOpts) (common.Address, error) {
			address, _, _, err := contract.DeployValidator(auth, sim, beneficiaries, signers, prefunds, "DRKCHAIN", "DRK", 18, genesisPrefundAddr, genesisPrefundBalance)
			return address, err
		})
		if err != nil {
		...
		genesis.Alloc[params.ValidatorContract] = core.GenesisAccount{
			Balance: new(big.Int).Mul(prefund, big.NewInt(int64(len(signers)))),
			Code:    code,
			Storage: storage,
		}
	default:
