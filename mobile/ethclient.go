...
// GetFeeAt returns the wei fee of the given account.
// The block number can be <0, in which case the balance is taken from the latest known block.
func (ec *EthereumClient) GetFeeAt(ctx *Context, account *Address, number int64) (balance *BigInt, _ error) {
	if number < 0 {
		rawFee, err := ec.client.FeeAt(ctx.context, account.address, nil)
		return &BigInt{rawFee}, err
	}
	rawFee, err := ec.client.FeeAt(ctx.context, account.address, big.NewInt(number))
	return &BigInt{rawFee}, err
}
